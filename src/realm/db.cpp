...
class CopyToReplication : public Replication {
public:
    CopyToReplication(TransactionRef tr)
        : m_tr(tr)
    {
    }
    ...
    {
        m_current.table = t;
        auto table = get_destination_table();
    ...
    void create_object_with_primary_key(const Table* t, ObjKey key, Mixed primary_key) override
    {
        ...
        m_current.obj = get_destination_table()->create_object_with_primary_key(primary_key);
    }

    void set(const Table* t, ColKey col_key, ObjKey key, Mixed val, _impl::Instruction) override
    {
        sync(t, key);
        auto dest_col_key = get_destination_colkey(col_key);
        if (val.is_type(type_Link, type_TypedLink)) {
            auto target_table = t->get_opposite_table(col_key);
            auto dest_table = get_destination_table();
            auto dest_target_table = dest_table->get_opposite_table(dest_col_key);
            if (auto pk_col = dest_target_table->get_primary_key_column()) {
                auto target_obj = target_table->get_object(val.get<ObjKey>());
                auto pk = target_obj.get_any(pk_col);
                val = Mixed(dest_target_table->get_objkey_from_primary_key(pk));
            }
            else {
                ...
                if (auto key = m_current.obj.get<ObjKey>(dest_col_key)) {
                    m_current.obj = dest_target_table->get_object(key);
                }
                else {
                    // If not, create one
                    m_current.obj = m_current.obj.create_and_set_linked_object(col_key);
                }
                return;
            }
        }
        m_current.obj.set_any(dest_col_key, val);
    }

    void list_clear(const CollectionBase& coll) override
    {
        sync(coll);
        auto dest_col_key = get_destination_colkey(coll.get_col_key());
        m_current.obj.get_listbase_ptr(dest_col_key)->clear();
    }
    ...
    void list_insert(const CollectionBase& coll, size_t idx, Mixed value, size_t) override
    {
        ...
        sync(coll);
        auto dest_col_key = get_destination_colkey(col_key);
        auto list = m_current.obj.get_listbase_ptr(dest_col_key);
        if (value.is_type(type_Link, type_TypedLink)) {
            auto target_table = m_current.table->get_opposite_table(col_key);
            auto dest_table = get_destination_table();
            auto dest_target_table = dest_table->get_opposite_table(dest_col_key);
            if (auto pk_col = dest_target_table->get_primary_key_column()) {
                auto target_obj = target_table->get_object(value.get<ObjKey>());
                auto pk = target_obj.get_any(pk_col);
                value =
                    Mixed(ObjLink(dest_target_table->get_key(), dest_target_table->get_objkey_from_primary_key(pk)));
            }
            else {
                REALM_ASSERT(dest_target_table->is_embedded());
                // Push state and switch to child which will be processed next
                m_states.push_back(m_current);
                m_current.obj_key = value.get<ObjKey>();
                m_current.table = target_table.unchecked_ptr();
                auto link_list = m_current.obj.get_linklist(dest_col_key);
                // We know that list has been cleared before inserting members, so there is
                // no former value to check.
                m_current.obj = link_list.create_and_insert_linked_object(idx);
                return;
            }
        }
    ...
    void set_insert(const CollectionBase& coll, size_t, Mixed value) override
    {
        ...
        sync(coll);
        auto dest_col_key = get_destination_colkey(col_key);
        auto set = m_current.obj.get_setbase_ptr(dest_col_key);
        if (value.is_type(type_Link, type_TypedLink)) {
            auto target_table = m_current.table->get_opposite_table(col_key);
            auto dest_table = get_destination_table();
            auto dest_target_table = dest_table->get_opposite_table(dest_col_key);
            auto pk_col = dest_target_table->get_primary_key_column();
            REALM_ASSERT(pk_col); // We can't have set of embedded objects
            auto target_obj = target_table->get_object(value.get<ObjKey>());
            auto pk = target_obj.get_any(pk_col);
            value = Mixed(ObjLink(dest_target_table->get_key(), dest_target_table->get_objkey_from_primary_key(pk)));
        }
    ...
    void dictionary_insert(const CollectionBase& coll, size_t, Mixed key, Mixed value) override
    {
        ...
        sync(coll);
        auto dest_col_key = get_destination_colkey(col_key);
        auto dict = m_current.obj.get_dictionary(dest_col_key);
        if (value.is_type(type_Link, type_TypedLink)) {
            auto target_table = m_current.table->get_opposite_table(col_key);
            auto dest_table = get_destination_table();
            auto dest_target_table = dest_table->get_opposite_table(dest_col_key);
            if (auto pk_col = dest_target_table->get_primary_key_column()) {
                ...
                if (tmp && tmp->is_same_type(value)) {
                    ObjKey key = tmp->get<ObjKey>();
                    m_current.obj = dest_target_table->get_object(key);
                }
                else {
                    // If not, create one
                    m_current.obj = dict.create_and_insert_linked_object(key);
                }
                return;
            }
        }
...
private:
    ...
    ColKey get_destination_colkey(ColKey c)
    {
        auto col_name = m_current.table->get_column_name(c);
        return get_destination_table()->get_column_key(col_name);
    }
    ...
    void sync(const Table* t, ObjKey obj_key)
    {
        if (t != m_current.table || obj_key != m_current.obj_key) {
            ...
            auto pk_col = t->get_primary_key_column();
            REALM_ASSERT(pk_col);
            auto pk = obj.get_any(pk_col);
            m_current.table = t;
            m_current.obj_key = obj_key;
            m_current.obj = get_destination_table()->get_object_with_primary_key(pk);
        }
    }
    TransactionRef m_tr;
    struct State {
