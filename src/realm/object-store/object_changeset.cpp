...
bool ObjectChangeSet::modifications_contains(ObjKey obj, const std::vector<ColKey>& filtered_column_keys) const
{
    ...
    std::unordered_set<ColKey> changed_columns_for_object = m_modifications.at(obj);
    for (const auto& column_key_in_filter : filtered_column_keys) {
        for (const auto& column_key_in_modifications : changed_columns_for_object) {
            if (column_key_in_modifications == column_key_in_filter) {
